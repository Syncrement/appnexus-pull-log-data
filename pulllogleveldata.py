...
import json, requests, os, hashlib, sys, time, pickle, getopt, math, configparser, re
...
def getAvailableLogs(cookieJar, filter, updatedSince):
		logListUrl = 'http://api.appnexus.com/siphon'
		
		# set optional parameters
		params = {}
		if filter and filter != "":
			params["siphon_name"] = filter
		if updatedSince:
...
def main (argv):
		...
		def read_config(configFileAbs):
			nonlocal username, password, memberId, dataDir, requestsPerMin, awsAccessKeyId, awsSecret, awsRegion, minTimePerRequestInSecs
		...
		for opt, arg in opts:
				if opt in ("-h", "--help"):
						...
						## simply ignore, we already processed the -c option
						pass
				else:
						print(usage.format(dataDir))
						sys.exit()
		...
		# Do the work
		#
		print("Running " + __file__ + " with this configuration:\n \
[LoginData]\n \
username={0}\n \
password=*******\n \
memberId={1}\n \
...
filter={6}\n \
updatesSince={7}\n \
\n".format(username, memberId, dataDir,awsAccessKeyId,awsRegion, requestsPerMin, filter, updateSince))
		...
		try:
				...
				if cookieJar:
						...
						logFiles = getAvailableLogs(cookieJar, filter, updateSince)
						if logFiles:
